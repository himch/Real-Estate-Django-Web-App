...
class DatabaseDownloader(Downloader, SQLExecutor):
    ...
    async def load_data_into_db(self):
        ...
        for i, offer in enumerate(self.database_dict['realty-feed']['offers']):
            ...
            exists = await sync_to_async(Listing.objects.filter(complex_id=int(offer['complex-id'])).exists())
            if exists:
                listing = await sync_to_async(Listing.objects.filter(complex_id=int(offer['complex-id'])))
                ...
                if offer_updated_at != offer['updated_at']:
                    ...
                    print(
                        f"Offer with complex-id {offer['complex-id']} exist with the different updated_at {offer_updated_at} != {offer['updated_at']}, deleted")
                else:
                    print(
                        f"Offer with complex-id {offer['complex-id']} exist with the same updated_at {offer['updated_at']}")
